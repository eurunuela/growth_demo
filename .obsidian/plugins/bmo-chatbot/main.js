...
function displayEditButton(settings, userP) {
...
function displayAppendButton(message) {
...
function displayBotMessage(settings, messageHistory2, message) {
  ...
  botMessageDiv.style.backgroundColor = colorToHex(settings.botMessageBackgroundColor || getComputedStyle(document.body).getPropertyValue(DEFAULT_SETTINGS.botMessageBackgroundColor).trim());
...
async function fetchOpenAIAPI(settings, index) {
  var _a2, _b, _c, _d, _e, _f;
  ...
  try {
    ...
    if (messageContainerEl2) {
      const botMessages = messageContainerEl2.querySelectorAll(".botMessage");
...
async function fetchOpenAIBaseAPI(settings, index) {
  const openai = new openai_default({
    apiKey: settings.apiKey,
    baseURL: settings.openAIBaseUrl,
    dangerouslyAllowBrowser: true
...
async function ollamaFetchData(settings, index) {
...
async function ollamaFetchDataStream(settings, index) {
  const ollamaRestAPIUrl = settings.ollamaRestAPIUrl;
  if (!ollamaRestAPIUrl) {
    return;
  }
...
async function openAIRestAPIFetchData(settings, index) {
  let prompt = await getPrompt(settings);
  if (prompt == void 0) {
    prompt = "";
  }
...
async function openAIRestAPIFetchDataStream(settings, index) {
...
async function requestUrlAnthropicAPI(settings, index) {
  const headers = {
...
function filterMessageHistory(messageHistory2) {
  const filteredMessageHistory = messageHistory2.filter((message, index, array) => {
    var _a2, _b;
...
function createBotMessage(settings) {
  const messageContainer = document.querySelector("#messageContainer");
  const botMessage = document.createElement("div");
  botMessage.classList.add("botMessage");
  botMessage.style.backgroundColor = colorToHex(
    settings.botMessageBackgroundColor || getComputedStyle(document.body).getPropertyValue(DEFAULT_SETTINGS.botMessageBackgroundColor).trim()
  );
...
function displayMessage(messageBlock, messageHtml, settings) {
...
async function commandModel(input, settings, plugin) {
  ...
  if (input.split(" ")[1] !== void 0) {
    ...
    if (Object.entries(modelAliases).find(([key, val]) => key === inputModel)) {
      settings.model = modelAliases[inputModel];
      messageHtml = `<div class="formattedSettings"><p><strong>Updated Model to ${settings.model}</strong></p></div>`;
    } else if (Object.entries(modelAliases).find(([key, val]) => val === inputModel)) {
      settings.model = modelAliases[Object.keys(modelAliases).find((key) => modelAliases[key] === inputModel) || ""];
      messageHtml = `<div class="formattedSettings"><p><strong>Updated Model to ${settings.model}</strong></p></div>`;
    } else {
...
async function commandPrompt(input, settings, plugin) {
  ...
  if (!input.split(" ")[1]) {
    ...
    let currentPrompt = settings.prompt.replace(/\.[^/.]+$/, "");
    if (!currentPrompt) {
      currentPrompt = "Empty";
    }
    const formattedSettings = `<div class="formattedSettings">
  ...
  if (input.startsWith("/prompt ")) {
    ...
    if (promptAliases[inputValue]) {
      settings.prompt = promptAliases[inputValue] + ".md";
      currentModel = settings.prompt.replace(/\.[^/.]+$/, "");
...
async function commandReference(input, settings, plugin) {
  ...
  if (inputValue === "true" || inputValue === "on") {
    settings.allowReferenceCurrentNote = true;
    ...
    settings.allowReferenceCurrentNote = false;
    if (referenceCurrentNoteElement) {
...
async function commandTemperature(input, settings, plugin) {
  ...
  if (settings && !isNaN(floatValue) && floatValue >= 0 && floatValue <= 1) {
    settings.temperature = parseFloat((Math.round(floatValue / 0.05) * 0.05).toFixed(2));
    temperatureSettingMessage = `${settings.temperature}`;
  } else {
...
async function commandMaxTokens(input, settings, plugin) {
  ...
  if (maxTokensValue !== void 0 && maxTokensValue !== "") {
    ...
    if (!isNaN(inputValue) && inputValue >= 0) {
      settings.max_tokens = inputValue.toString();
...
async function commandSystem(input, settings, plugin) {
  ...
  if (systemPromptValue !== null) {
    if (systemPromptValue[1]) {
      settings.system_role = systemPromptValue[1];
      formattedSettings += `
              <div class="formattedSettings">
...
async function commandSave(settings) {
  let folderName = settings.chatHistoryPath;
  ...
  try {
    ...
    const templateFile = allFiles.find((file2) => file2.path.toLowerCase() === settings.templateFilePath.toLowerCase());
    if (templateFile) {
...
`;
    ...
    let fileName = "";
    if (settings.allowRenameNoteTitle) {
...
function displayUserMessage(settings, message) {
  ...
  userMessageDiv.style.backgroundColor = colorToHex(settings.userMessageBackgroundColor || getComputedStyle(document.body).getPropertyValue(DEFAULT_SETTINGS.userMessageBackgroundColor).trim());
...
var BMOView = class extends import_obsidian6.ItemView {
  ...
  async BMOchatbot() {
    ...
    if (!this.settings.apiKey && OPENAI_MODELS.includes(this.settings.model)) {
      new import_obsidian6.Notice("API key not found. Please add your OpenAI API key in the plugin settings.");
      ...
      if (OPENAI_MODELS.includes(this.settings.model)) {
        try {
          ...
          new import_obsidian6.Notice("Error occurred while fetching completion: " + error.message);
          console.log(error.message);
        }
        ...
        new import_obsidian6.Notice("No models detected.");
      }
    }
  }
  async onClose() {
...
async function fetchOpenAIRestAPIModels(plugin) {
  ...
  if (lastError) {
    console.error("Error making API request:", lastError);
    new import_obsidian7.Notice("OpenAI REST API URL connection error.");
    throw lastError;
  }
}

// src/components/settings/GeneralSettings.ts
async function addGeneralSettings(containerEl, plugin, SettingTab9) {
  ...
  new import_obsidian8.Setting(settingsContainer).setName("Model").setDesc("Choose a model.").addDropdown(async (dropdown) => {
    ...
    if (plugin.settings.openAIRestAPIUrl != "") {
      ...
      try {
        openAIRestAPIModels.forEach((model) => {
          dropdown.addOption(model, model);
          if (!plugin.settings.allModels.includes(model)) {
            plugin.settings.allModels.push(model);
          }
  ...
  new import_obsidian8.Setting(settingsContainer).setName("System").setDesc("System role prompt.").addTextArea(
    (text) => text.setPlaceholder("You are a helpful assistant.").setValue(plugin.settings.system_role !== void 0 ? plugin.settings.system_role : DEFAULT_SETTINGS.system_role).onChange(async (value) => {
  ...
  new import_obsidian8.Setting(settingsContainer).setName("Temperature").setDesc("Temperature controls how random the generated output is. Lower values make the text more predictable, while higher values make it more creative and unpredictable.").addSlider(
  ...
  new import_obsidian8.Setting(settingsContainer).setName("Allow Reference Current Note").setDesc("Allow chatbot to reference current active note during conversation.").addToggle(
    (toggle) => toggle.setValue(plugin.settings.allowReferenceCurrentNote).onChange((value) => {
...
function addAppearanceSettings(containerEl, plugin, SettingTab9) {
  const toggleSettingContainer = containerEl.createDiv({ cls: "toggleSettingContainer" });
  ...
  new import_obsidian9.Setting(settingsContainer).setName("Background Color for User Messages").setDesc("Modify the background color of the userMessage element.").addButton(
  ...
  new import_obsidian9.Setting(settingsContainer).setName("Background Color for Bot Messages").setDesc("Modify the background color of the botMessage element.").addButton(
    (button) => button.setButtonText("Restore Default").setIcon("rotate-cw").setClass("clickable-icon").onClick(async () => {
  ...
  new import_obsidian9.Setting(settingsContainer).setName("Allow Header").setDesc("Display chatbot name and model name in header.").addToggle(
    (toggle) => toggle.setValue(plugin.settings.allowHeader).onChange((value) => {
...
function addChatHistorySettings(containerEl, plugin, SettingTab9) {
  const toggleSettingContainer = containerEl.createDiv({ cls: "toggleSettingContainer" });
...
// src/components/settings/OllamaSettings.ts
function addOllamaSettings(containerEl, plugin, SettingTab9) {
  const toggleSettingContainer = containerEl.createDiv({ cls: "toggleSettingContainer" });
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("mirostat").setDesc("Enable Mirostat sampling for controlling perplexity. (default: 0, 0 = disabled, 1 = Mirostat, 2 = Mirostat 2.0)").addText(
    (text) => text.setPlaceholder("0").setValue(plugin.settings.ollamaParameters.mirostat || DEFAULT_SETTINGS.ollamaParameters.mirostat).onChange(async (value) => {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("mirostat_eta").setDesc("Influences how quickly the algorithm responds to feedback from the generated text. A lower learning rate will result in slower adjustments, while a higher learning rate will make the algorithm more responsive. (Default: 0.1)").addText(
    (text) => text.setPlaceholder("0.1").setValue(plugin.settings.ollamaParameters.mirostat_eta || DEFAULT_SETTINGS.ollamaParameters.mirostat_eta).onChange(async (value) => {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("mirostat_tau").setDesc("Controls the balance between coherence and diversity of the output. A lower value will result in more focused and coherent text. (Default: 5.0)").addText(
    (text) => text.setPlaceholder("5.0").setValue(plugin.settings.ollamaParameters.mirostat_tau || DEFAULT_SETTINGS.ollamaParameters.mirostat_tau).onChange(async (value) => {
      const floatValue = parseFloat(value);
      if (!isNaN(floatValue)) {
        if (parseInt(value) === floatValue) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("num_ctx").setDesc("Sets the size of the context window used to generate the next token. (Default: 2048)").addText(
    (text) => text.setPlaceholder("2048").setValue(plugin.settings.ollamaParameters.num_ctx || DEFAULT_SETTINGS.ollamaParameters.num_ctx).onChange(async (value) => {
      const intValue = parseInt(value, 10);
      if (isNaN(intValue)) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("num_gqa").setDesc("The number of GQA groups in the transformer layer. Required for some models, for example it is 8 for llama2:70b.").addText(
    (text) => text.setPlaceholder("0").setValue(plugin.settings.ollamaParameters.num_gqa || DEFAULT_SETTINGS.ollamaParameters.num_gqa).onChange(async (value) => {
      const intValue = parseInt(value, 10);
      if (isNaN(intValue)) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("num_thread").setDesc("Sets the number of threads to use during computation. By default, Ollama will detect this for optimal performance. It is recommended to set this value to the number of physical CPU cores your system has (as opposed to the logical number of cores).").addText(
    (text) => text.setPlaceholder("0").setValue(plugin.settings.ollamaParameters.num_thread || DEFAULT_SETTINGS.ollamaParameters.num_thread).onChange(async (value) => {
      const intValue = parseInt(value, 10);
      if (isNaN(intValue)) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("repeat_last_n").setDesc("Sets how far back for the model to look back to prevent repetition. (Default: 64, 0 = disabled, -1 = num_ctx)").addText(
    (text) => text.setPlaceholder("64").setValue(plugin.settings.ollamaParameters.repeat_last_n || DEFAULT_SETTINGS.ollamaParameters.repeat_last_n).onChange(async (value) => {
      const intValue = parseInt(value, 10);
      if (isNaN(intValue)) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("repeat_penalty").setDesc("Sets how strongly to penalize repetitions. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. (Default: 1.1)").addText(
    (text) => text.setPlaceholder("1.1").setValue(plugin.settings.ollamaParameters.repeat_penalty || DEFAULT_SETTINGS.ollamaParameters.repeat_penalty).onChange(async (value) => {
      const floatValue = parseFloat(value);
      if (!isNaN(floatValue)) {
        if (parseInt(value) === floatValue) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("seed").setDesc("Sets the random number seed to use for generation. Setting this to a specific number will make the model generate the same text for the same prompt.").addText(
    (text) => text.setPlaceholder("0").setValue(plugin.settings.ollamaParameters.seed || DEFAULT_SETTINGS.ollamaParameters.seed).onChange(async (value) => {
      const intValue = parseInt(value, 10);
      if (isNaN(intValue)) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("stop").setDesc("Sets the stop sequences to use. When this pattern is encountered, the LLM will stop generating text and return. Multiple stop patterns may be set by specifying them as a comma-separated list in the input field.").addText(
    (text) => text.setPlaceholder("stop, \\n, user:").setValue(plugin.settings.ollamaParameters.stop && Array.isArray(plugin.settings.ollamaParameters.stop) ? plugin.settings.ollamaParameters.stop.join(", ") : DEFAULT_SETTINGS.ollamaParameters.stop.join(", ")).onChange(async (value) => {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("tfs_z").setDesc("Tail free sampling is used to reduce the impact of less probable tokens from the output. A higher value (e.g., 2.0) will reduce the impact more, while a value of 1.0 disables this setting. (default: 1)").addText(
    (text) => text.setPlaceholder("1.0").setValue(plugin.settings.ollamaParameters.tfs_z || DEFAULT_SETTINGS.ollamaParameters.tfs_z).onChange(async (value) => {
      const floatValue = parseFloat(value);
      if (!isNaN(floatValue)) {
        if (parseInt(value) === floatValue) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("top_k").setDesc("Reduces the probability of generating nonsense. A higher value (e.g. 100) will give more diverse answers, while a lower value (e.g. 10) will be more conservative. (Default: 40)").addText(
    (text) => text.setPlaceholder("40").setValue(plugin.settings.ollamaParameters.top_k || DEFAULT_SETTINGS.ollamaParameters.top_k).onChange(async (value) => {
      const intValue = parseInt(value, 10);
      if (isNaN(intValue)) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("top_p").setDesc("Works together with top-k. A higher value (e.g., 0.95) will lead to more diverse text, while a lower value (e.g., 0.5) will generate more focused and conservative text. (Default: 0.9)").addText(
    (text) => text.setPlaceholder("1.0").setValue(plugin.settings.ollamaParameters.top_p || DEFAULT_SETTINGS.ollamaParameters.top_p).onChange(async (value) => {
      const floatValue = parseFloat(value);
      if (!isNaN(floatValue)) {
        if (parseInt(value) === floatValue) {
  ...
  new import_obsidian11.Setting(advancedSettingsContainer).setName("keep_alive").setDesc("If set to a positive duration (e.g. 20m, 1hr or 30), the model will stay loaded for the provided duration in seconds. If set to a negative duration (e.g. -1), the model will stay loaded indefinitely. If set to 0, the model will be unloaded immediately once finished. If not set, the model will stay loaded for 5 minutes by default.").addText(
    (text) => text.setPlaceholder("30s").setValue(plugin.settings.ollamaParameters.keep_alive || DEFAULT_SETTINGS.ollamaParameters.keep_alive).onChange(async (value) => {
...
function addAPIConnectionSettings(containerEl, plugin, SettingTab9) {
  const toggleSettingContainer = containerEl.createDiv({ cls: "toggleSettingContainer" });
  ...
  new import_obsidian12.Setting(settingsContainer).setName("API Key").setDesc("Insert API Key.").addText(
  ...
  new import_obsidian12.Setting(settingsContainer).setName("OPENAI BASE URL").setDesc("Enter your custom OpenAI-base url.").addButton(
    (button) => button.setButtonText("Restore Default").setIcon("rotate-cw").setClass("clickable-icon").onClick(async () => {
...
function addPromptSettings(containerEl, plugin, SettingTab9) {
  const toggleSettingContainer = containerEl.createDiv({ cls: "toggleSettingContainer" });
  ...
  new import_obsidian13.Setting(settingsContainer).setName("Prompt Folder Path").setDesc("Reference your prompts from a specified folder.").addText(
  ...
  new import_obsidian13.Setting(settingsContainer).setName("Prompt").setDesc("Select a prompt.").addDropdown((dropdown) => {
...
function addOpenAIRestAPIUrlSettings(containerEl, plugin, SettingTab9) {
  const toggleSettingContainer = containerEl.createDiv({ cls: "toggleSettingContainer" });
  ...
  new import_obsidian14.Setting(settingsContainer).setName("OPENAI REST API URL").setDesc("Enter your custom OPENAI REST API url.").addText(
    (text) => text.setPlaceholder("http://localhost:1234").setValue(plugin.settings.openAIRestAPIUrl || DEFAULT_SETTINGS.openAIRestAPIUrl).onChange(async (value) => {
  ...
  new import_obsidian14.Setting(settingsContainer).setName("Allow Stream").setDesc(addDescriptionLink("Allow custom OPENAI REST API models to stream response. Additional setup required: ", "https://github.com/longy2k/obsidian-bmo-chatbot/wiki/How-to-setup-with-LM-Studio", "", "[Instructions]")).addToggle(
    (toggle) => toggle.setValue(plugin.settings.allowOpenAIRestAPIStream).onChange((value) => {
...
async function addEditorSettings(containerEl, plugin, SettingTab9) {
  const toggleSettingContainer = containerEl.createDiv({ cls: "toggleSettingContainer" });
  ...
  new import_obsidian15.Setting(settingsContainer).setName("Prompt Select Generate System").setDesc(addDescriptionLink("System role for Prompt Select Generate.", "https://github.com/longy2k/obsidian-bmo-chatbot/wiki/Prompt---Select---Generate-Command", "", "[Instructions]")).addTextArea(
    (text) => text.setPlaceholder("You are a helpful assistant.").setValue(plugin.settings.system_role_prompt_select_generate !== void 0 ? plugin.settings.system_role_prompt_select_generate : DEFAULT_SETTINGS.system_role_prompt_select_generate).onChange(async (value) => {
      plugin.settings.system_role_prompt_select_generate = value !== void 0 ? value : DEFAULT_SETTINGS.system_role_prompt_select_generate;
      await plugin.saveSettings();
    })
  );
}
...
// src/settings.ts
var BMOSettingTab = class extends import_obsidian16.PluginSettingTab {
...
async function openAIRestAPIFetchDataEditor(settings, selectionString) {
  const urls = [
    settings.openAIRestAPIUrl + "/v1/chat/completions",
    settings.openAIRestAPIUrl + "/api/v1/chat/completions"
  ];
...
async function renameTitleCommand(settings) {
  ...
  let fileContent = "";
  try {
    new import_obsidian18.Notice("Generating title...");
    if (activeFile) {
    ...
    new import_obsidian18.Notice("Renamed note title.");
  } catch (error) {
    console.error(error);
  }
}
async function promptSelectGenerateCommand(settings) {
  ...
  if (view && select && select.trim() !== "") {
    if (OPENAI_MODELS.includes(settings.model)) {
      try {
        new import_obsidian18.Notice("Generating...");
        ...
        new import_obsidian18.Notice("Error occurred while fetching completion: " + error.message);
        console.log(error.message);
      }
    } else if (ANTHROPIC_MODELS.includes(settings.model)) {
      try {
    ...
    new import_obsidian18.Notice("Generation complete.");
  } else if (settings.openAIBaseModels.includes(settings.model)) {
    try {
      new import_obsidian18.Notice("Generating...");
...
var BMOGPT9 = class extends import_obsidian19.Plugin {
  async onload() {
